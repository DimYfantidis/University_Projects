...
b=fir1(250,[58 62]/(Fs/2),'stop');
fir_x=filtfilt(b,1,x); % apply the filter in zero-phase mode 
subplot(3,1,2),plot(time,fir_x),grid,title('FIR stopband-filtered signal')

% design an stop-band IIR 3-order filter for removing powerline nosie @ 60 Hz 
[b,a]=butter(3,[58 62]/(Fs/2),'stop');
iir_x=filtfilt(b,a,x); % apply the filter in zero-phase (bidirectional) mode 
subplot(3,1,3),plot(time,iir_x),grid,title('butterworth IIR stopband-filtered signal')
