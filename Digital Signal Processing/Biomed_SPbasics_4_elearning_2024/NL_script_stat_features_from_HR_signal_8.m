...
figure(1),clf
subplot(3,2,1),plot([1:numel(hr_pre)],hr_pre,'r'),title('before'),ylabel('beats/min'),
subplot(3,2,3),plot([1:numel(hr_med)],hr_med,'g'),title('during'),ylabel('beats/min'),
subplot(3,2,5),plot([1:numel(hr_unknown)],hr_unknown,'b'),title('unknown'),ylabel('beats/min'),xlabel('time')

%%%%%%%% extracting statistical features 
Features_pre=[mean(hr_pre),std(hr_pre),skewness(hr_pre)]
Features_med=[mean(hr_med),std(hr_med),skewness(hr_med)]
Features_unknown=[mean(hr_unknown),std(hr_unknown),skewness(hr_unknown)]
