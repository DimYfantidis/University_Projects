...
class FullyConnectedClassifier:
    def __init__(
        ...
        learning_rate: float, 
        error_func: Literal["cross-entropy", "square-error", "L2-SVM"], 
        ...
        elif error_func == "L2-SVM":
            self.compute_loss = self.l2_svm
        else:
    ...
    def l2_svm(self, d: np.ndarray, y: np.ndarray):
        id_true = np.where(d == 1)[0][0]
        result = np.zeros(shape=(self.m, 1), dtype=self.dtype)
        for j in range(self.m):
            if j == id_true:
                continue
            result[id_true][0] += -2 * max(0, 1 + y[j][0] - y[id_true][0])
    
        for j in range(self.m):
            if j == id_true:
                continue
            result[j][0] = 2 * max(0, 1 + y[j][0] - y[id_true][0])
        return result
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray, test: None | tuple[np.ndarray, np.ndarray] = None, n_epochs: int = 1):
        ...
        try:
            for epoch in range(1, n_epochs+1):
                ...
                stop = timer()
                # Log results
                train_acc = self.score(X_train, Y)
                test_acc = self.score(X_test, Y_test)
                print(f"Finished epoch No.{epoch}, Elapsed Time: {stop-start:.2f}sec",end='')
                print(f", Train Accuracy: {train_acc * 100:.2f}%", end='')
                if test is not None:
                    print(f", Test Accuracy: {test_acc * 100:.2f}%", end='')
                print()
