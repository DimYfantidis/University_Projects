...
// FILE IS ALMOST IDENTICAL TO srtf.c
...
process* extract_min(list* THIS, int compare_processes(process*, process*))
{
		...
		iterator = iterator->next)
    {
        ...
        // In the case of SJF, the comparator examines the relation between p1's and p2's burst times.
	}
...
// ----------- Process List Implementation (END) -----------


//  Implementation of SJF scheduling algorithm (non-preemptive)
// 		@param P: array of processes for scheduling
//		@param n: array size
void shortest_job_first(process* P, int n)
{
    /* Double-ended list data structure for keeping track of the processes that
     * have arrived and are awaiting execution (ready). By SJF's definition
     * the process with the minimum burst time will be selected from the list.
     *
     * Normally, it should have been a priority queue but taking the time to implement
	...
	while (completed_processes < n)
    {
        ...
        while (process_next->burst_time > 0)
        {
